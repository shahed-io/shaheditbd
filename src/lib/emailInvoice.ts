  ...
  wrapper.innerHTML = `
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:28px">
      ...
      <div style="background:${brandLight};border-radius:10px;padding:16px;border-left:4px solid ${brandColor}">
        ...
        <p style="font-size:12px;color:#555;margin:0 0 4px 0">Method: <strong style="color:#1a1a2e">${esc(pmLabel)}</strong></p>
        ${data.transactionId ? `<p style="font-size:12px;color:#555;margin:4px 0">TrxID: <strong style="color:#1a1a2e;font-family:monospace;background:#e8e5f7;padding:1px 6px;border-radius:4px;font-size:11px">${esc(data.transactionId)}</strong></p>` : ''}
        ${statusLabel ? `<p style="font-size:12px;color:#555;margin:4px 0">Status: <span style="background:${brandColor};color:#fff;padding:2px 10px;border-radius:999px;font-size:11px;font-weight:600">${esc(statusLabel)}</span></p>` : ''}
      </div>
    </div>
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;border-radius:8px;overflow:hidden">
      <thead><tr style="background:${brandColor}">
        <th style="color:#fff;font-size:11px;text-transform:uppercase;padding:12px 14px;text-align:center">#</th>
        <th style="color:#fff;font-size:11px;text-transform:uppercase;padding:12px 14px;text-align:left">পণ্যের নাম</th>
        <th style="color:#fff;font-size:11px;text-transform:uppercase;padding:12px 14px;text-align:center">পরিমাণ</th>
        <th style="color:#fff;font-size:11px;text-transform:uppercase;padding:12px 14px;text-align:right">দাম</th>
        <th style="color:#fff;font-size:11px;text-transform:uppercase;padding:12px 14px;text-align:right">মোট</th>
      </tr></thead>
      <tbody>${itemsHtml}</tbody>
    </table>
    <div style="display:flex;justify-content:flex-end;margin-bottom:24px">
      <div style="min-width:280px;background:${brandLight};border-radius:10px;padding:18px">
        <div style="display:flex;justify-content:space-between;font-size:13px;color:#555;margin-bottom:8px"><span>সাবটোটাল:</span><span>${fmt(sub)}</span></div>
        ${disc > 0 ? `<div style="display:flex;justify-content:space-between;font-size:13px;color:#059669;margin-bottom:8px"><span>ডিসকাউন্ট:</span><span>-${fmt(disc)}</span></div>` : ''}
        <div style="height:1px;background:${brandColor};opacity:0.3;margin:10px 0"></div>
        <div style="display:flex;justify-content:space-between;font-size:18px;font-weight:800;color:${brandColor}"><span>সর্বমোট:</span><span>${fmt(data.total)}</span></div>
      </div>
    </div>

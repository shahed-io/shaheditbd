...
export function installCopyDeterrents(opts: CopyDeterrentOptions = {}): () => void {
  ...
  const FUN_MESSAGES = [
    ...
    { icon: '🚫', title: 'উঁহু, এটা করা যাবে না!', sub: 'Shahed IT কনটেন্ট কপি করা নিষিদ্ধ।' },
    { icon: '🛡️', title: 'কনটেন্ট লক করা আছে', sub: 'কপি করতে চাইলে আমাদের সাথে যোগাযোগ করুন — আমরা friendly! 💜' },
    { icon: '🎯', title: 'নাইস ট্রাই!', sub: 'কিন্তু এই ওয়েবসাইটের ডিজাইন © Shahed IT সম্পত্তি।' },
    { icon: '✨', title: 'কপি না করে কিনে নিন!', sub: 'আসল প্রোডাক্ট পেতে অর্ডার করুন shahedit.com তে।' },
  ...
  const onCopy = (e: ClipboardEvent) => {
    if (isInteractive(e.target)) return;
    e.preventDefault();
    try { e.clipboardData?.setData('text/plain', '© Shahed IT — Content protected. https://shahedit.com'); } catch { /* ignore */ }
    warn();
  };

...
export const DEFAULT_COUNTRY = COUNTRIES.find(c => c.code === 'BD') || COUNTRIES[0];

/** Split a stored phone string into a country + local part (best effort). */
export const splitPhone = (value: string): { country: Country; local: string } => {
  const raw = (value || '').trim();
  if (raw.startsWith('+')) {
    const matches = COUNTRIES.filter(c => raw.startsWith(c.dial));
    if (matches.length) {
      const best = matches.reduce((a, b) => (b.dial.length > a.dial.length ? b : a));
      return { country: best, local: raw.slice(best.dial.length).replace(/[^0-9]/g, '') };
    }
  }
  return { country: DEFAULT_COUNTRY, local: raw.replace(/[^0-9]/g, '') };
};

export const joinPhone = (country: Country, local: string) => {

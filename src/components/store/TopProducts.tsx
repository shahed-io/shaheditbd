...
const TopProducts = () => {
  ...
  return (
    <section ref={sectionRef as React.RefObject<HTMLElement>} className="pt-6 pb-10 bg-transparent below-fold">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
          <div>
            ...
            <h2 className="section-heading text-3xl sm:text-4xl mt-3">
              Top{' '}
              <span style={{ background: 'linear-gradient(135deg, hsl(192,75%,59%), hsl(192,70%,58%))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }}>
                Selling
              </span>
            </h2>
        ...
        <div className="flex flex-wrap gap-2 mb-8 pb-8 border-b border-border">
          {tabs.map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              style={activeTab === tab ? { background: 'linear-gradient(135deg, hsl(192,75%,59%), hsl(192,70%,58%))' } : {}}
                ...
                <CategoryRevealBlock key={cat} catIdx={catIdx}>
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="font-sora font-bold text-lg text-foreground flex items-center gap-2">
                      <span className="w-1 h-5 rounded-full" style={{ background: 'linear-gradient(180deg, hsl(192,75%,59%), hsl(192,70%,58%))' }} />
                      {cat}

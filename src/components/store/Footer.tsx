import { Phone, Mail, Globe, Facebook, Instagram, MessageCircle, Shield, ExternalLink, ArrowUpRight, Zap, Package, Info, FileText, Send } from 'lucide-react';
  ...
  <footer className="relative overflow-hidden" style={{ background: 'hsl(var(--background))' }}>
    ...
    <div className="absolute inset-0 pointer-events-none overflow-hidden">
      <div className="absolute top-0 right-0 w-[700px] h-[600px]"
        style={{ background: 'radial-gradient(ellipse at 80% 0%, hsla(258,78%,55%,0.09), transparent 65%)' }} />
      <div className="absolute bottom-0 left-0 w-[600px] h-[500px]"
        style={{ background: 'radial-gradient(ellipse at 0% 100%, hsla(200,90%,45%,0.08), transparent 65%)' }} />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[700px]"
        style={{ background: 'radial-gradient(ellipse, hsla(162,72%,38%,0.04), transparent 70%)' }} />
      <div className="absolute inset-0"
        style={{ backgroundImage: 'radial-gradient(circle, hsla(258,78%,55%,0.04) 1px, transparent 1px)', backgroundSize: '28px 28px' }} />
    </div>

    {/* ── CTA Strip — hero-card-shell wrapper ── */}
    <div className="relative z-10">
      <div className="hero-card-shell mx-4 sm:mx-6 lg:mx-8 mt-8 mb-0 rounded-3xl">
        <div className="hero-card-inner relative px-6 py-6">
          {/* Shimmer top line */}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-5">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0"
                style={{ background: 'linear-gradient(135deg, hsl(258,78%,55%), hsl(200,90%,45%))', boxShadow: '0 4px 16px hsla(258,78%,55%,0.35)' }}>
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              <a href="https://wa.me/8801840099853" target="_blank" rel="noopener noreferrer"
                ...
                style={{
                  ...
                  boxShadow: '0 2px 12px hsla(226,35%,12%,0.08)',
                }}>
              ...
              <a href="/shop"
                ...
                style={{ background: 'linear-gradient(135deg, hsl(258,78%,55%), hsl(200,90%,45%))', boxShadow: '0 4px 20px hsla(258,78%,55%,0.38)' }}>
    ...
    <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-8">
      ...
      {/* Brand section — hero-card-shell */}
      <div className="hero-card-shell mb-5 rounded-3xl">
        <div className="hero-card-inner relative flex flex-col items-center text-center space-y-5 px-6 py-7">
          {/* Shimmer top */}
          <div className="absolute top-0 left-0 right-0 h-px rounded-full"
            style={{ background: 'linear-gradient(90deg, transparent 0%, rgba(255,255,255,1) 50%, transparent 100%)' }} />
          {/* Ambient glow dots */}
          <div className="absolute top-4 left-8 w-2 h-2 rounded-full opacity-40"
            style={{ background: 'hsl(258,78%,55%)', filter: 'blur(6px)' }} />
          <div className="absolute top-4 right-8 w-2 h-2 rounded-full opacity-40"
            style={{ background: 'hsl(200,90%,45%)', filter: 'blur(6px)' }} />
            ...
            {[
              ...
              <a key={i} href={s.href} target="_blank" rel="noopener noreferrer"
                ...
                onMouseEnter={e => {
                  ...
                  el.style.boxShadow = `0 4px 16px ${s.color.replace('hsl(','hsla(').replace(')',',0.22)')}`;
                }}
                onMouseLeave={e => {
                  const el = e.currentTarget as HTMLElement;
                  el.style.color = 'hsl(226,35%,42%)';
                  el.style.borderColor = 'hsla(258,78%,75%,0.25)';
      ...
      {/* Nav columns — each wrapped in cta-card-shell */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {NAV_COL.map((col, ci) => (
          <div key={ci} className="cta-card-shell">
            <div className="cta-card-inner relative space-y-4">
              {/* Shimmer top */}
              <div className="absolute top-0 left-0 right-0 h-px rounded-full"
                style={{ background: 'linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.95) 50%, transparent 100%)' }} />
              {/* Column header */}
              <div className="flex items-center gap-2.5">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                  style={{
                    background: col.accent.replace('hsl(','hsla(').replace(')',',0.12)'),
                    border: `1.5px solid ${col.accent.replace('hsl(','hsla(').replace(')',',0.30)')}`,
                    color: col.accent,
                    boxShadow: `0 2px 8px ${col.accent.replace('hsl(','hsla(').replace(')',',0.15)')}`,
                  }}>
                  {col.icon}
                </div>
              ...
              <div className="h-px rounded-full"
                style={{ background: `linear-gradient(90deg, ${col.accent.replace('hsl(','hsla(').replace(')',',0.50)')}, transparent)` }} />
                  ...
                  <li key={link.label}>
                    <a href={link.href} target={(link as any).external ? '_blank' : undefined}
                      ...
                      <span className="w-1.5 h-1.5 rounded-full flex-shrink-0 transition-all group-hover:scale-150"
                        style={{ background: col.accent.replace('hsl(','hsla(').replace(')',',0.50)') }} />
    ...
    {/* ── Trust bar — hero-card-shell ── */}
    <div className="relative z-10 mx-4 sm:mx-6 lg:mx-8 mb-6">
      <div className="hero-card-shell rounded-3xl">
        <div className="hero-card-inner relative px-5 py-4">
          {/* Shimmer top */}
          <div className="absolute top-0 left-0 right-0 h-px rounded-full"
            style={{ background: 'linear-gradient(90deg, transparent 0%, rgba(255,255,255,1) 50%, transparent 100%)' }} />
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <div className="w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 overflow-hidden"
                style={{ background: 'hsl(0,0%,100%)', boxShadow: '0 4px 12px hsla(258,78%,55%,0.20)', border: '1px solid hsla(258,78%,65%,0.22)' }}>
              ...
              {[
                ...
                <span key={pm.name}
                  ...
                  style={{
                    ...
                    boxShadow: '0 1px 4px hsla(258,78%,55%,0.08)',
                  }}
                  title={pm.num}>
                  {pm.name}
                </span>

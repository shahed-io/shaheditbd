...
const NAV_COL = [
  { title: 'Products', links: [{ label: 'Windows Keys', href: '#' }, { label: 'Office 365', href: '#' }, { label: 'Adobe Creative', href: '#' }, { label: 'Streaming', href: '#' }, { label: 'VPN & Security', href: '#' }, { label: 'AI Tools', href: '#' }] },
  { title: 'Information', links: [{ label: 'Blog', href: '/blog' }, { label: 'Help Center', href: '/help' }, { label: 'About Us', href: '#' }, { label: 'My Account', href: '/dashboard' }, { label: 'Contact Us', href: '#' }, { label: 'All Products', href: '#' }] },
  { title: 'Policies', links: [{ label: 'Privacy Policy', href: '/help' }, { label: 'Terms & Conditions', href: '/help' }, { label: 'Refund Policy', href: '/help' }, { label: 'Order Policy', href: '/help' }, { label: 'Delivery Info', href: '/help' }, { label: 'Return Policy', href: '/help' }] },
];
  ...
  <footer className="relative overflow-hidden" style={{ background: 'hsl(var(--background))' }}>
    ...
    <div className="relative z-10 border-b" style={{ borderColor: 'hsl(220,20%,90%)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col sm:flex-row items-center justify-between gap-5">
        <div>
          ...
          <p className="text-sm mt-1 text-muted-foreground">Our experts are available 24/7 to assist you</p>
        </div>
        <div className="flex items-center gap-3 flex-shrink-0">
          ...
          <a href="/shop" className="flex items-center gap-2 px-5 py-2.5 rounded-2xl text-sm font-bold text-white hover:opacity-90 transition-all"
    ...
    <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12">
      ...
      <div className="lg:col-span-2 space-y-6">
        <a href="/" className="flex items-center group w-fit"><BrandLogo size="lg" /></a>
        <p className="text-[13px] leading-relaxed max-w-xs text-muted-foreground">
          ...
          {[
            ...
            <a key={i} href={c.href} target={c.ext ? '_blank' : undefined} rel={c.ext ? 'noopener noreferrer' : undefined}
              ...
              <span className="w-7 h-7 rounded-xl flex items-center justify-center flex-shrink-0 transition-all group-hover:scale-105"
                style={{ background: 'hsla(0,0%,100%,0.85)', backdropFilter: 'blur(8px)', border: '1px solid hsl(220,20%,88%)' }}>
                {c.icon}
              </span>
        ...
        <div className="flex gap-2">
          {[
            ...
            <a key={i} href={s.href} target="_blank" rel="noopener noreferrer"
              ...
              style={{ background: 'hsla(0,0%,100%,0.80)', backdropFilter: 'blur(8px)', border: '1px solid hsl(220,20%,88%)', color: 'hsl(226,35%,42%)' }}
              title={s.label}
      ...
      {NAV_COL.map((col, ci) => (
        <div key={ci}>
          <h4 className="font-sora font-bold text-[11px] uppercase tracking-[0.18em] mb-5 flex items-center gap-2"
            ...
            <span className="w-5 h-[2px] rounded-full" style={{ background: 'linear-gradient(90deg, hsl(258,78%,55%), hsl(200,90%,45%))' }} />
            {col.title}
          </h4>
          <ul className="space-y-2.5">
            {col.links.map(link => (
              <li key={link.label}>
                <a href={link.href} className="text-[13px] transition-colors flex items-center gap-1.5 group"
                  style={{ color: 'hsl(226,35%,45%)' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.color = 'hsl(258,78%,50%)'; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.color = 'hsl(226,35%,45%)'; }}>
                  <span className="w-0 group-hover:w-3 overflow-hidden transition-all duration-200"><ArrowRight size={11} /></span>
                  {link.label}
                </a>
              </li>
            ))}
    ...
    <div className="relative z-10 border-t" style={{ borderColor: 'hsl(220,20%,88%)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {['bKash', 'Nagad', 'Rocket', 'উপায়', 'bKash Merchant'].map(pm => (
            <span key={pm} className="px-2.5 py-1.5 rounded-lg text-[10px] font-bold font-fira"
              style={{ background: 'hsla(0,0%,100%,0.85)', backdropFilter: 'blur(8px)', color: 'hsl(226,35%,28%)', border: '1px solid hsl(220,20%,88%)' }}>
              {pm}
            </span>
          ))}
        </div>

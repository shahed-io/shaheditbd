...
const AdminHeroBanner = () => {
  ...
  const addFeature = (slideId: string) => {
    setSlides(prev => prev.map(s =>
      s.id === slideId ? { ...s, features: [...s.features, 'New Feature'] } : s
    ));
  };

  const removeFeature = (slideId: string, idx: number) => {
    setSlides(prev => prev.map(s =>
      s.id === slideId ? { ...s, features: s.features.filter((_, i) => i !== idx) } : s
    ));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-muted-foreground text-sm">হোমপেজের স্লাইডার ও ব্যাকগ্রাউন্ড কাস্টমাইজ করুন</p>
        </div>
      ...
      <div className="flex gap-2">
        {([['slides', 'স্লাইডস', LayoutTemplate], ['background', 'ব্যাকগ্রাউন্ড', Palette]] as const).map(([tab, label, Icon]) => (
              ...
              {editingSlide === slide.id && (
                <div className="border-t border-border/50 p-5 space-y-5 bg-muted/10">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Tag */}
                    <Field label="ট্যাগ (Tag)" value={slide.tag}
                      onChange={v => updateSlide(slide.id, 'tag', v)} placeholder="🔥 Best Seller" />
                    {/* Badge */}
                    <Field label="ব্যাজ (Badge)" value={slide.badge}
                      onChange={v => updateSlide(slide.id, 'badge', v)} placeholder="MOST POPULAR" />
                    {/* Title */}
                    <Field label="টাইটেল (Title)" value={slide.title}
                      onChange={v => updateSlide(slide.id, 'title', v)} placeholder="Windows 11" />
                    {/* Title Accent */}
                    <Field label="টাইটেল অ্যাকসেন্ট" value={slide.titleAccent}
                      onChange={v => updateSlide(slide.id, 'titleAccent', v)} placeholder="Pro" />
                    {/* Subtitle */}
                    <Field label="সাবটাইটেল" value={slide.subtitle}
                      onChange={v => updateSlide(slide.id, 'subtitle', v)} placeholder="Original License Key" />
                    {/* Emoji */}
                    <Field label="ইমোজি" value={slide.emoji}
                      onChange={v => updateSlide(slide.id, 'emoji', v)} placeholder="🪟" />
                    {/* Price */}
                    <Field label="মূল্য (Price)" value={slide.price}
                      onChange={v => updateSlide(slide.id, 'price', v)} placeholder="৳599" />
                    {/* Original */}
                    <Field label="আসল মূল্য (Original)" value={slide.original}
                      onChange={v => updateSlide(slide.id, 'original', v)} placeholder="৳9,999" />
                    {/* Off */}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="flex gap-2 items-center">
                        <input type="color"
                          value={hslToHex(slide.accentFrom)}

...
const ShopProductCard = ({ product }: { product: Product }) => {
  ...
  return (
    <div
      ...
      style={{ border: '1.5px solid hsl(var(--border))', background: 'hsl(var(--background))', boxShadow: '0 2px 12px hsla(258,78%,55%,0.06)' }}
      ...
      <div className="relative aspect-square overflow-hidden bg-muted">
        ...
        {product.badge && (
          <span className="absolute top-2 right-2 text-[10px] font-bold text-white px-2 py-1 rounded-full"
            style={{ background: 'linear-gradient(135deg, hsl(258,78%,55%), hsl(200,90%,45%))' }}>{product.badge}</span>
        )}
      </div>
      {/* Info */}
      <div className="p-3">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            onClick={e => { e.stopPropagation(); addToCart({ id: product.id, name: product.name, price: product.price, image: product.image_url || '' }); }}
              ...
              {categories.map(cat => {
                ...
                return (
                  <button key={cat.id} onClick={() => setCategory(cat.slug)}
                    ...
                    {meta.icon} <span className="ml-1">{cat.name}</span>
                  </button>
                );
              })}
            </div>

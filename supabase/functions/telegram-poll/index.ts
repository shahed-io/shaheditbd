...
// Status keyword mapping (Bengali + English)
...
async function sendTelegramReply(botToken: string, chatId: string | number, text: string, replyToMessageId?: number) {
  ...
  try {
    ...
    while (true) {
      ...
      for (const update of updates) {
        ...
        // Look up the order from the message mapping
        ...
        if (!mapping) {
          console.log('No order mapping for message:', replyMessageId);
          continue;
        }
        ...
        if (orderErr || !order) {
          await sendTelegramReply(BOT_TOKEN, chatId, `❌ অর্ডার #${mapping.order_number} খুঁজে পাওয়া যায়নি।`, msg.message_id);
          continue;
        }

        if (order.status === newStatus) {
          const label = STATUS_LABELS[newStatus] || newStatus;
          await sendTelegramReply(BOT_TOKEN, chatId, `ℹ️ অর্ডার #${mapping.order_number} ইতিমধ্যে "${label}" স্ট্যাটাসে আছে।`, msg.message_id);
          continue;
        }
        ...
        if (updateErr) {
          ...
          await sendTelegramReply(BOT_TOKEN, chatId, `❌ অর্ডার #${mapping.order_number} আপডেট ব্যর্থ: ${updateErr.message}`, msg.message_id);
          continue;
        }
        ...
        const confirmMsg = `${emoji} অর্ডার স্ট্যাটাস আপডেট সফল!\n\n` +
        ...
        await sendTelegramReply(BOT_TOKEN, chatId, confirmMsg, msg.message_id);
        totalProcessed++;
        console.log(`Order #${mapping.order_number} status: ${order.status} → ${newStatus}`);
      }
